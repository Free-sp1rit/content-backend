...
- [x] 增加文章阅读计数 Redis 原型，学习 Redis 原子计数器。
- [x] 补充 Redis 运行验证和边界测试。
- [x] 增加登录用户阅读去重 Redis 原型，学习 `SET NX EX`。
- [x] 完成 Redis 阅读计数原子化与验证收口，学习 Lua 脚本。
- [ ] 补充部署 runbook、环境验证和 smoke 验证。
- [ ] 加固文章发布/编辑并发场景。
- [ ] 阶段末增加最小 Web 前端验收界面。
...
- `000-Tie-up-loose-ends-before-next-phase.md` 已完成。
- `001-redis-atomic-counter-integration` 的代码实现、测试和 Redis 文档更新已完成，等待收口提交。
- `002-redis-counter-runtime-verification.md` 已补 Redis 阅读计数运行验证和边界测试。
- 学习笔记按 `003-redis-counter-learning-notes.md` 单独整理，不阻塞当前代码收口。
- `004-redis-authenticated-view-dedup.md` 已完成 Redis `SET NX EX` 和可选认证学习实践。
- `005-redis-view-counter-atomicity-tie-up.md` 已完成 Redis 阅读计数原子化和验证收口。
- 下一阶段建议回到部署文档与 smoke 验证对齐。
- 后续任务草稿应根据学习推进结果继续生成或调整，不必保留已经失效的旧编号文件。
